# Harness — обвязка надёжности (v4)

> v4 (скилл project-setup v8+). Эволюция: v1.0 — сессионный цикл закрытия (4 слоя);
...
> v4 — **живость сессий по ПРОЦЕССУ, не по таймеру** (`session_liveness.py`): соперник = живой
> claude-процесс этой папки (по cwd/inode), а не маркер моложе 2ч. Чинит ложные «сессия открыта»
> на долгих/мобильных сессиях; `/end` самочинится (sweep мёртвых) и называет соперника поимённо;
> гарантия слияния инбоксов (Блок 5 + гейт на старте). Маркер хранит root-PID (строка 2).
...
`session_start_recovery.sh` → `.claude/hooks/`. При старте: (а) если есть `.session_incomplete.json` — через `additionalContext` принудительно инструктирует Claude восстановить wrap-up; (б) пишет маркер своей сессии (строка1=время, строка2=**root-PID** claude) в реестр `.claude/live_sessions/` и подметает маркеры **мёртвых процессов** (`session_liveness.py sweep` — точность kill-уровня, не таймер); (в) **гейт слияния инбоксов**: если в `System/memory/inbox_sessions/` есть несмёрженные дельты — через `additionalContext` требует слить их ДО работы (Пакет C); (г) авто-убирает stale `.git/index.lock` (>30 мин без живого git-процесса).
...
- **Страж параллельности (v4 — живость по процессу):** соперник = живой claude-процесс этой папки (`session_liveness.py blocking_rivals`: cwd/inode-матчинг, не таймер 2ч → не ломается на долгих/мобильных сессиях). При записи в общие файлы памяти при живом сопернике — deny с инструкцией записать дельту в `System/memory/inbox_sessions/` (заголовок «## для [файл]») и **именем соперника** (для проверки «реальная или забытая»). Владельцу — ноль кнопок. Scheduled-прогоны исключены (бесконфликтны по дизайну). Fail-open: сбой определения живости → запись пропускается.
