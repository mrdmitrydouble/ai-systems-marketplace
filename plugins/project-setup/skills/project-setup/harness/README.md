# Harness — обвязка надёжности (v3)

> v3 (скилл project-setup v7+). Эволюция: v1.0 — сессионный цикл закрытия (4 слоя);
> v2 — + страж параллельности, health-скрипт, автопересборка индекса;
> v3 — + frozen zones и слияние инбоксов в memory_write_guard, фиксы хуков.
...
4. `System/memory/STARTUP.md` — шаги обработки `.session_incomplete.json` и слияния `inbox_sessions/` (в шаблоне STARTUP v8 из STRUCTURES.md это шаги 0.2 и 0.3 — уже на месте).
